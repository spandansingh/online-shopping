				...
				<div class="col-lg-6 pull-right seller">
					<?php 
						if (($name = $this -> Session -> read('username')) == "") {
							echo $this -> Html -> link('Login', array('controller' => 'Users', 'action' => 'login'), array('class' => 'pull-right seller'));
						} 
					?>
					...
					<?php
						...
						if(($this->Session->read('role')=='') && ($this->Session->read('username')!=''))
							echo $this->Html->link('Wishlist',array('controller'=>'Wishes','action'=>'viewwishes'));
					?>
